...
import { styled, Chip, ChipProps, FormControl } from '@mui/material';
...
const StyledChip = styled(Chip)(({ theme }) => ({
  ...
  '&.default:not(.hasError)': {
    backgroundColor: theme.palette.mode === 'light' ? SFGrey[100] : SFGrey[500],
      ...
      '& .MuiChip-deleteIcon': {
        '& .MuiIconButton-label': {
          '& svg': {
            '& path': {
              fill: `${
                theme.palette.mode === 'light' ? SFGrey[400] : SFGrey[900]
              } !important`
            }
          }
        }
    ...
    <FormControl fullWidth={fullWidth}>
      <StyledChip
        ...
        className={`${className} ${sfColor} ${fullWidth ? 'fullWidth' : ''} ${
          hasError ? 'hasError' : ''
        } ${!clickable ? 'SFChip-clickDisabled' : ''}`}
        label={label}
        size={size}
        variant={variant}
        disabled={disabled}
