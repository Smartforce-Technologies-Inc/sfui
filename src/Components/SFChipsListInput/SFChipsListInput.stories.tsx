...
import {
  SFChipsListInput,
  SFChipsListInputProps,
  ValueType
} from './SFChipsListInput';

const SFChiplistInputOptions = [
...
export default {
  ...
  component: SFChipsListInput,
  args: {
    label: 'Bagel',
    options: [], // SFChiplistInputOptions
    emptyMessage: 'There are no permissions assigned yet.',
    items: [
      { value: 'name1@company.com' },
      { value: 'name2@company.com' },
      { value: 'name3@company.com' }
    ]
  },
  argTypes: {
    onChange: {
...
const Template: Story<SFChipsListInputProps> = (args) => {
  return (
    <div
      style={{
        display: 'flex',
    ...
    >
      <SFChipsListInput
        {...args}
        onChange={(newItems: ValueType[]): void => console.log(newItems)}
      />
    </div>
  );
};
